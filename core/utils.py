...
def send_mail(from_email, recipient_list, attachments=[]):
    ...
    email = get_templated_mail('mail', {
        'image': InlineImage('image.jpeg', open('./static/registration.jpeg', 'rb').read(), 'jpeg'),
...
def send_mail_to_user(template, from_email, user):
    ...
    email = get_templated_mail(template, {
        'image': InlineImage(filename="registration.jpg",
                             content=open('./static/registration.jpg', 'rb').read()),
        'user': user,
    }, from_email, recipient_list)
